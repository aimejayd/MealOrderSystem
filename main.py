...
navigator_symbol = "/"  # This will make the program runnable on any unix based enviroument because
# it has different file system
if os.name == "nt":
...
def def_main():
    while True:  # Repeat Menu until stops.
        ...
        input_1 = str(input(
            "Please Select Your Operation: ")).upper()  # Input, have to choose operation. Make everything
        # UPPER symbol.
...
def def_file_sorter():  # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(
            list_drinks[i].index("RM") + 3):])  # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(
            list_services[i].index("RM") + 3):])  # Applying extraction on 80 and above items wich are Services
        i += 1
